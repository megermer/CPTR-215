...
class SetCard:
    ...
    def third_card_helper(element1, element2, enum):
        """
        >>> SetCard.third_card_helper(Color.RED, Color.RED, Color)
        <Color.RED: 1>
        >>> SetCard.third_card_helper(Shape.OVAL, Shape.PYRAMID, Shape)
        <Shape.QUAD: 5>
        """
        if element1 == element2:
            return element1
        else:
            return list(set(enum) - {element1} - {element2})[0]
    ...
    def third_card(self, other):
        ...
        3FGQ
        '''
        number = SetCard.third_card_helper(self.number, other.number, [1, 2, 3])
        fill = SetCard.third_card_helper(self.fill, other.fill, Fill)
        color = SetCard.third_card_helper(self.color, other.color, Color)
        shape = SetCard.third_card_helper(self.shape, other.shape, Shape)
